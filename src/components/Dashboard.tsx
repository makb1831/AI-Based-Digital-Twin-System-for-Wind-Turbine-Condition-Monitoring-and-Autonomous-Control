...
export default function Dashboard({ data, history }: DashboardProps) {
  ...
  return (
    <div className="bg-slate-900/50 border border-white/10 p-6 rounded-xl shadow-xl flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard label="Temperature" value={data.temperature.toFixed(1)} unit="°C" status={data.temperature > 85 ? 'warning' : data.temperature > 100 ? 'critical' : 'optimal'} />
        <MetricCard label="Vibration" value={data.vibration.toFixed(2)} unit="mm/s" status={data.vibration > 4 ? 'warning' : data.vibration > 6 ? 'critical' : 'optimal'} />
      </div>

      <div className="h-64 mt-4 bg-slate-950/50 rounded-lg p-4 border border-white/5 relative">
        <div className="absolute top-4 left-4 z-10 text-xs font-semibold text-slate-400">Power Output (MW)</div>
